...
def add_habit(habit, creation_time=None):
    ...
    cursor.execute("INSERT INTO Habit(FKUserID, Name, Periodicity, CreationTime) VALUES (?, ?, ?, ?)",
                   (user_id[0], habit.name, habit.periodicity, creation_time))
    db.commit()
    # TODO: Sicherstellen, dass User nicht schon ein Habit mit demselben Namen hat


def return_habit_id(db, habit):
    """
...
def return_surrounding_streaks(db, habit, check_date):
    """
    :param habit: the habit which is to be checked off
    :param db: the database containing the data
    ...
    """
    habit_id = return_habit_id(db, habit)
    period_start = an.determine_period_start(habit.periodicity, check_date)
    previous_period_start = an.determine_previous_period_start(habit.periodicity, period_start)
    next_period_start = an.determine_next_period_start(habit.periodicity, check_date)
...
def return_streak_name(db, habit, check_date):
    """
    determines the streak to which the current completion belongs
    :param habit: the habit for which the streak name is to be returned
    :param db: the database containing the data
    ...
    habit_id, previous_streak, current_streak, next_streak = \
        return_surrounding_streaks(db, habit, check_date)
    cursor = db.cursor()
...
def rename_streaks(db, habit, check_date):
    ...
    if current_streak != next_streak and next_streak is not None:
        ...
        while next_period_existing:
            ...
            db.commit()
            next_period_start = an.determine_next_period_start(habit.periodicity, next_period_start)
            next_streak = sql_streak_name(db, next_period_start, habit_id)
...
def add_period(habit, check_date=None):
    ...
    habit_id = return_habit_id(db, habit)
    period_start = an.determine_period_start(habit.periodicity, check_date)
    streak_name = return_streak_name(db, habit, check_date)
    ...
    rename_streaks(db, habit, check_date)
    db.commit()
